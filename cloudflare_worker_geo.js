...
// ISO-3166 alpha-2 → friendly country name (request.cf.country gives the code,
// e.g. 'RO'; we show 'Romania'). Covers the common OF-traffic countries; any
// code not listed falls back to the raw code.
const COUNTRY_NAMES = {
  US:'United States', GB:'United Kingdom', CA:'Canada', AU:'Australia', NZ:'New Zealand',
  IE:'Ireland', DE:'Germany', FR:'France', IT:'Italy', ES:'Spain', PT:'Portugal',
  ...
  EE:'Estonia', MD:'Moldova', TR:'Turkey', RU:'Russia', BR:'Brazil', MX:'Mexico',
  AR:'Argentina', CL:'Chile', CO:'Colombia', PE:'Peru', JP:'Japan', KR:'South Korea',
  CN:'China', HK:'Hong Kong', TW:'Taiwan', SG:'Singapore', MY:'Malaysia', TH:'Thailand',
  PH:'Philippines', ID:'Indonesia', VN:'Vietnam', IN:'India', PK:'Pakistan',
  AE:'United Arab Emirates', SA:'Saudi Arabia', IL:'Israel', ZA:'South Africa',
  NG:'Nigeria', EG:'Egypt', MA:'Morocco'
};

// ─── GEO landing page (2026-06-09) ─────────────────────────────────────────
// The human-facing page for the cloak-geo worker: full-bleed 9:16 image (random
// per visit), the model's name, the visitor's OWN city (request.cf.city — free),
// "available now", a heartbeat CTA, and a countdown. The CTA goes through the
// SAME /r→/v→/go chain as the link-tree worker, so the OF link is never exposed
// to crawlers. Reuses the proven model lookup + image route; only the template
// is new.
function geoLandingHTML(model, incomingUtm) {
...
<style>
  ...
  *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
  ...
  /* ── In-app-browser escape (SAME mechanic as the old links) ──
     Tooltip + 18+ interstitial with the pink "Open in external browser"
     button. Shown ONLY when JS detects an in-app webview AND the user taps
     the CTA. Real browsers skip it and go straight through /r → /v → /go. */
...
<script>
  (function(){
    ...
    function tick(){var left=dl-Date.now();if(left<=0){dl=Date.now()+DURATION;localStorage.setItem(KEY,String(dl));left=DURATION;}var m=Math.floor(left/60000),s=Math.floor((left%60000)/1000);el.textContent=m+':'+(s<10?'0':'')+s;}
...
(function(){
  // ── In-app-browser escape — SAME mechanic as the old links ──
  //   • Real browser (Safari/Chrome): the CTA → /r → /v → /go chain plays
  //     out naturally. No overlay, no escape attempt.
  //   • In-app webview (IG/FB/TikTok/etc): the CTA tap is intercepted and
  //     the 18+ overlay shows. From it the user can (a) tap the pink button
  //     → per-platform scheme (IG/Threads single-tap, Android intent://,
  //     iOS-FB scheme-cascade then HOLD) → chain plays out externally; or
  //     (b) tap ⋯ → "Open in external browser" → landing reopens in the OS
  //     browser → re-tap → real-browser flow.
  ...
  if (goBtn && cta) {
    // Pink button escape — IG/Threads private scheme (single tap), Android
    // intent://, iOS-FB/other scheme cascade then transform to hold-to-Open.
...
<meta name="theme-color" content="#000000">
<style>
  :root { --accent:#ff2d8f; --img:url('${img}'); }
  *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
  html,body{margin:0;height:100%;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;background:#0b0b0d}
  .bg-blur{position:fixed;inset:0;background-image:var(--img);background-size:cover;background-position:center;filter:blur(34px) brightness(.55) saturate(1.25);transform:scale(1.15);z-index:0}
  .stage{position:relative;z-index:1;height:100dvh;aspect-ratio:9/16;max-width:100vw;margin:0 auto;background-image:var(--img);background-size:cover;background-position:center 22%;overflow:hidden;box-shadow:0 0 60px rgba(0,0,0,.55)}
...
<body>
  ...
  <div class="stage">
    <div class="panel">
      ...
      <div class="status"><span class="dot">🟢</span> Available now</div>
      <p class="blurb">I live near <b>${here}</b>, let's get to know each other 🥰</p>
      <a class="cta" id="cta" href="/r?${utmQS}" rel="noopener">SEND ME A MESSAGE ❤️</a>
    </div>
  </div>
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === '/go' || url.pathname === '/go/') {
        ...
        if (!_slug) {
          // Path-mode: the slug rides through as ?s=<slug>, set on the
          // geo CTA href (/go?s=<slug>&...). Restores main-domain
          // (path-mode) geo links that carry no subdomain label.
          const _sp = (url.searchParams.get('s') || '').toLowerCase();
          if (/^[a-z0-9_-]{2,40}$/.test(_sp)) _slug = _sp;
          else {
            // Tertiary recovery (matches original fb2eb39): utm_campaign
            // carries the slug; KV lookup below still validates.
            const _uc = (url.searchParams.get('utm_campaign') || '').toLowerCase();
            if (/^[a-z0-9_-]{2,40}$/.test(_uc)) _slug = _uc;
          }
        }
        if (!_slug) {
      ...
      // GEO worker: the visitor's OWN city/country (free, from Cloudflare) +
      // a RANDOM card image. The batch wizard stores N images under
      // img:<slug>:0 … img:<slug>:N-1 and the count under img:<slug>:count.
      model.city    = (request.cf && request.cf.city)    || '';
      model.country = (request.cf && request.cf.country) || '';
      const _sl = (slug || '').toLowerCase();
      let _imgN = 0;
      try { _imgN = parseInt((await env.SLUG_MAP.get(`img:${_sl}:count`)) || '0', 10) || 0; }
      catch (e) { _imgN = 0; }
      if (_imgN > 0) {
